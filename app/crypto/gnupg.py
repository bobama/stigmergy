...
def _write_passphrase(stream, passphrase, encoding):
    passphrase = '%s\n' % passphrase
    passphrase = passphrase.encode(encoding) # if the encoding is enforced, random byte passwords throw errors.
...
class GPG(object):
    ...
    def import_keys(self, key_data, keyring=None):
        ...
        args = [ "--import" ]
        if keyring:
            args.append("--secret-keyring %s.sec --keyring %s.pub --no-default-keyring" % (keyring,keyring))
    ...
    def encrypt_file(self, file, recipients, sign=None,
        ...
        if always_trust:
            args.append("--trust-model always --no-auto-check-trustdb")
        if hidekeyid:
    ...
    def decrypt_file(self, file, always_trust=False, passphrase=None,
                     output=None, secretkey=None, keyring=None):
        args = ["--decrypt"]
        if output:  # write the output to a file with the specified name
            if os.path.exists(output):
                os.remove(output) # to avoid overwrite confirmation message
        ...
        if always_trust:
            args.append("--trust-model always --no-auto-check-trustdb")
        if secretkey:
            # This might have to be --try-secret-key instead, depending on the gpg version.
            args.append('--default-key "%s"' % secretkey)
        if keyring:
            args.append("--secret-keyring %s.sec --keyring %s.pub --no-default-keyring" % (keyring,keyring))
        result = self.result_map['crypt'](self)
